...
9. **XFA Forms (Dynamic Adobe LiveCycle)**: `is_xfa_pdf`, `extract_xfa_fields` - Schema extraction for dynamic XFA forms that no open-source library can render.

   **The governing invariant (v2.3.1): absence and failure never share a representation.** `is_xfa_pdf` has three outcomes, not two: `is_xfa=True` with an `xfa_type`, `is_xfa=False` for a readable non-XFA PDF, and `is_xfa=None` with `detection_failed=True` when the file could not be read at all. **Callers must check `detection_failed` before trusting `is_xfa`.** v2.3.0 collapsed the third case into `False`, which sent damaged form packages down the AcroForm path to the exact cryptic "document closed" this feature existed to eliminate. Same rule downstream: an unparseable XFA template raises rather than reporting zero fields, and an unknown `profile` is an error rather than a silent fallback to generic.

   `extract_xfa_fields` parses the XFA template XML for field names, captions and UI types. Defaults to the **zipForm producer profile** (Lone Wolf / zipForm Plus conventions used by real-estate forms), matched case-insensitively so `"zipForm"` works. Pass `profile="generic"` plus `extra_plumbing_patterns` / `extra_positional_patterns` for other producers. Note the pattern-matching asymmetry: plumbing patterns are substring searches, positional patterns are anchored at the start.
